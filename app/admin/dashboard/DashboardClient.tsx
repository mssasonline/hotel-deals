...
function PageHeader() {
  return (
    <div className="mb-8">
      ...
      <p className="text-gray-400 text-sm mt-1">SelectedRoom admin console — all systems operational.</p>
    </div>
  );
}
              ...
              return (
                <tr key={b.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    <StatusBadge status={b.status} variant="booking" />
                  </td>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
        ...
        <a href="/admin/reports" className="text-xs text-brand-blue font-semibold hover:underline">Reports →</a>
      </div>
      <div className="px-6 py-4 space-y-4">
        {cities.length === 0 ? (
          <p className="text-sm text-gray-400 py-4 text-center">No data yet.</p>
        ) : cities.map((city) => {
          const pct = (city.booking_count / max) * 100;
          return (
            <div key={city.city}>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <div className="text-right">
                  <span className="text-xs font-semibold text-gray-900">
                    {Number(city.booking_count).toLocaleString()} bookings
                  </span>
                  <span className="text-xs text-gray-400 ml-2"><AEDAmount amount={city.revenue} /></span>
                </div>
              </div>
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100">
        <h2 className="font-semibold text-gray-900">Active Customer Deals</h2>
      </div>
        ...
        <a href="/admin/last-min-rooms" className="px-6 py-5 flex items-center justify-between hover:bg-gray-50/50 transition-colors group">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-9 h-9 rounded-xl bg-indigo-50 flex items-center justify-center shrink-0">
              <svg className="w-4.5 h-4.5 text-indigo-500 w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <div>
              <p className="text-sm font-semibold text-gray-900">Last Min Rooms</p>
              <p className="text-xs text-gray-400 mt-0.5">Time-based discount rooms</p>
            </div>
          </div>
        ...
        <a href="/admin/deals" className="px-6 py-5 flex items-center justify-between hover:bg-gray-50/50 transition-colors group">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">Active partner deals</p>
            </div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
        ...
        <div className="sm:col-span-2 bg-[#001E5A] rounded-2xl p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="bg-white/8 rounded-xl px-5 py-4 text-center">
              <p className="text-brand-gold font-bold text-xl">{fmt(stats.revenue_today)}</p>
