...
export default async function AdminFinancialsPage() {
  ...
  const partnerHotels = new Map<string, Set<string>>();

  for (const b of rawRows) {
    const key = b.partner_name;
    if (!byPartner.has(key)) {
      byPartner.set(key, { partnerId: key, partnerName: b.partner_name, hotelCount: 0, bookingCount: 0, grossCollected: 0, platformCommission: 0, netPayout: 0 });
      partnerHotels.set(key, new Set());
    }
    const entry = byPartner.get(key)!;
    partnerHotels.get(key)!.add(b.hotel_id);
    entry.bookingCount       += 1;
    entry.grossCollected     += b.total_price;
    entry.platformCommission += b.admin_amount;
    entry.netPayout          += b.partner_amount;
    entry.hotelCount          = partnerHotels.get(key)!.size;
  }
    ...
    <div className="p-6 lg:p-8 max-w-7xl">
      ...
      <div className="mb-8 rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg, #0A1A4F 0%, #0F2260 50%, #1A3A8F 100%)', boxShadow: '0 4px 24px rgba(15,34,96,0.18)' }}>
        <div className="px-6 py-5">
          ...
          <p className="text-white/45 text-xs pl-3">Platform-wide monthly revenue, fee breakdown, and partner payouts</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        {([
          ...
          { label: 'Platform Revenue',     value: totals.platformCommission,      isAmount: true, blue: true },
          { label: 'Total Partner Payouts', value: totals.netPayout,              isAmount: true, green: true },
        ] as const).map(({ label, value, isAmount, blue, green }: {
          label: string; value: string | number; isAmount: boolean; blue?: boolean; green?: boolean
        }) => (
          <div key={label} className="bg-white rounded-2xl p-5" style={{ border: '1px solid rgba(30,58,138,0.09)', boxShadow: '0 2px 12px rgba(15,34,96,0.06)' }}>
            ...
            <p className={`text-2xl font-bold ${blue ? 'text-brand-blue' : green ? 'text-emerald-600' : 'text-gray-900'}`}>
      ...
      {/* ── Monthly Financial Summary (same table as partner) ─────────────────── */}
      <div className="bg-white rounded-2xl overflow-hidden mb-8" style={{ border: '1px solid rgba(30,58,138,0.08)', boxShadow: '0 2px 12px rgba(15,34,96,0.06)' }}>
        ...
        {months.length === 0 ? (
          <div className="p-12 text-center text-gray-400 text-sm">No paid bookings yet.</div>
        ) : (
          <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100 text-xs font-semibold text-gray-500 uppercase tracking-wide text-right">
                    ...
                    <th className="px-4 py-3 text-emerald-700">Partner Payouts</th>
                ...
                <tbody className="divide-y divide-gray-50">
                  {months.map((m) => (
                    <tr key={m.month} className="hover:bg-gray-50/50 transition-colors text-right">
                      ...
                      <td className="px-4 py-3 text-amber-600 font-medium tabular-nums"><AEDAmount amount={m.platformCommission} /></td>
                ...
                <tfoot>
                  <tr className="bg-gray-50 border-t-2 border-gray-200 text-right font-bold">
                    ...
                    <td className="px-4 py-3 text-emerald-700 text-base tabular-nums"><AEDAmount amount={totals.netPayout} /></td>
                    <td />
                  </tr>
                </tfoot>
              </table>
