...
function AddHotelModal({
  ...
  onClose: () => void;
  onCreated: (hotel: PartnerHotel) => void;
}) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">
                {t['partner.hotels.cityLabel']} <span className="text-red-400">*</span>
              </label>
              <input value={form.city} onChange={e => setForm(p => ({ ...p, city: e.target.value }))} className={INPUT} placeholder="Dubai" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">
                {t['partner.hotels.countryLabel']} <span className="text-red-400">*</span>
              </label>
          ...
          <div>
            ...
            <input value={form.address} onChange={e => setForm(p => ({ ...p, address: e.target.value }))} className={INPUT} placeholder="Sheikh Zayed Road" />
          </div>
          <div>
...
export default function HotelsPage() {
  ...
  function handleCreated(hotel: PartnerHotel) {
      ...
      <div className="mb-8 rounded-2xl overflow-hidden" style={{ background: 'linear-gradient(135deg, #0A1A4F 0%, #0F2260 50%, #1A3A8F 100%)', boxShadow: '0 4px 24px rgba(15,34,96,0.18)' }}>
        <div className="px-6 py-5 flex items-center justify-between gap-4">
          ...
          <button
            ...
            className="shrink-0 flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white rounded-xl transition-all hover:-translate-y-0.5"
            style={{ background: 'linear-gradient(135deg, #D97706 0%, #B45309 100%)', boxShadow: '0 2px 10px rgba(180,83,9,0.35)' }}
          >
