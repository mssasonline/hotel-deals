-- Migration 080: Split bookings_this/prev_month into Last Min Rooms vs Hotels Deals
...
RETURNS TABLE (
  ...
  active_deals          BIGINT,
  growth_pct            NUMERIC,
  lastmin_this_month    BIGINT,
  lastmin_prev_month    BIGINT,
  deals_this_month      BIGINT,
  deals_prev_month      BIGINT
)
LANGUAGE sql
STABLE
AS $$
  ...
  SELECT
    ...
    -- Last Min Rooms: deal_id IS NULL
    (SELECT COUNT(*) FROM bookings
      WHERE status != 'cancelled' AND deal_id IS NULL
    ...
    -- Hotels Deals: deal_id IS NOT NULL
    (SELECT COUNT(*) FROM bookings
      WHERE status != 'cancelled' AND deal_id IS NOT NULL
        AND date_trunc('month', created_at) = date_trunc('month', NOW()))::BIGINT,
    (SELECT COUNT(*) FROM bookings
      WHERE status != 'cancelled' AND deal_id IS NOT NULL
        AND date_trunc('month', created_at) = date_trunc('month', NOW() - INTERVAL '1 month'))::BIGINT
